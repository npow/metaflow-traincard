...
def _build_hf_callback_class():
    """
    Build and return HFTrainCardCallback, importing transformers at call time.

    Using a factory keeps transformers/torch out of the module-level import
    path so that merely importing metaflow_traincard (e.g. during Metaflow
    card discovery) does not pay the torch startup cost.
    """
    try:
...
def __getattr__(name: str):
    if name == "HFTrainCardCallback":
        cls = _build_hf_callback_class()
        globals()["HFTrainCardCallback"] = cls
        return cls
    raise AttributeError(f"module 'metaflow_traincard.adapters.huggingface' has no attribute {name!r}")
